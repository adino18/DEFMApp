angular.module('defmApp.services', ['LocalStorageModule'])
		...
		if (defaultConfig == null)
			defaultConfig = {
				...
				machine_host: {
					...
					defaultValue: 'http://192.168.1.100:81/',
					type: 'text',
					placeholder: 'TYPE_MACHINE_ADDRESS',
					show: true
				},
				maxCoorX: {
					key: 'maxCoorX',
					name: 'Max Coordinate X',
					defaultValue: '270',
					type: 'text',
					placeholder: 'MAX_COORDINATE_X',
					show: true
				},
				maxCoorY: {
					key: 'maxCoorY',
					name: 'Max Coordinate Y',
					defaultValue: '180',
					type: 'number',
					placeholder: 'MAX_COORDINATE_Y',
					show: true
				},
				feedRate: {
					key: 'feedRate',
	...
	.factory('Socket', ['Config', 'GCode', 'Canvas', "ngProgressFactory", "$ionicPopup", "$filter", "$ionicScrollDelegate", "$ionicUser", "$ionicPush", "$rootScope", "$http", "$cordovaFile", "$cordovaFileTransfer", "$state", function (Config, GCode, Canvas, ngProgressFactory, $ionicPopup, $filter, $ionicScrollDelegate, $ionicUser, $ionicPush, $rootScope, $http, $cordovaFile, $cordovaFileTransfer, $state) {
		...
		$rootScope.$on('$cordovaPush:tokenReceived', function (event, data) {
			...
			writeToCommandMonitor("Token:" +data);
			setToken(data.token);
		});
		...
		return {
			...
			//	mjpg_default_url = mjpg_default_url[0].concat(':' + mjpg_default_url[1]) + ":%s/?action=stream&t=%d";
				...
				setTimeout(function () {
					...
					uploader.addEventListener('choose', function (e) {
						console.log("SD"+$scope.dpi);
						$scope.dpi = 50;
						$scope.$apply();
						console.log("SD2: "+$scope.dpi);
						//scope.$apply();
						var files = e.files;
				...
				socket.on('settings', function (settings) {
					...
					Config.set('maxCoorX', intval(settings.maxCoorX))
					Config.set('maxCoorY', intval(settings.maxCoorY))
					scope.$apply();
				});
			...
			start: function (copies) {
				maxPoint = GCode.getMax();
				console.log("maxPoint:"+maxPoint);
				if(maxPoint.x > maxCoorX || maxPoint.y >maxCoorY){
						scope.alert("Image large than machine: "+maxCoorX+"x"+maxCoorY+" mm");;
						return;
				}
			...
			setMaxCoordicate: function (coorX,coorY) {
				maxCoorX = intval(coorX);
				maxCoorY = intval(coorY);
				socket.emit("maxCoordicate", maxCoorX,maxCoorY);
			},
			...
			//adino
			setTypeConvert2Svg: function (type) {
				socket.emit("convertToSvg", type);
			},
			setMachineHost: function (address) {
				socket.emit("machinehost", address);
			},
			setImageSize: function (size) {
				socket.emit("imageResize", size);
			},
			setManual: function(command){
				if(command == "home"){
					command ="G90 G0 X0 Y0 Z0";
	...
	.factory('Canvas', ['GCode', "$rootScope", function (GCode, $rootScope) {
		...
		function drawCoorsText(maxPoint,minPoint) {
			console.log("aaa"+height);
			console.log("maxy"+width);
			canvas.add(drawText(round(maxPoint.y, 3) + "mm", {left:5,top:height-1}));
			// canvas.add(drawText(round(minPoint.y, 3) + "mm", { left: 5, top: height - 10 }));
			canvas.add(drawText(" (Max-X: " + round(maxPoint.x, 3) + " mm; Max-Y: " + round(maxPoint.y, 3) + "mm)", { top: -1 }));
			//canvas.add(drawText(" (Min-X: " + round(minPoint.x, 3) + " mm; Min-Y: " + round(minPoint.y, 3) + "mm)", { top: -1 }));
			canvas.add(drawText(round(maxPoint.x, 3) + "mm", { originX: 'right', originY: 'bottom', top: 0, left: width }));
			//		canvas.add(drawText(round(minPoint.x, 3) + "mm", { originX: 'right', originY: 'bottom', top: 0, left: width-100 }));

	}

		function tryDraw(constConvert, maxPoint, processedLine,minPoint) {
			...
			minPoint = minPoint.multiply(1 / constConvert);
			//add text to tell user which line is x and the other is y
			drawCoorsText(maxPoint,minPoint);
			canvas.renderAll();
			return true;
		}
		...
		return {
			...
			create: function (processedLine) {
				...
				var minPoint = GCode.getMin().multiply(mm2px);
				constConvert = (maxPoint.x > maxPoint.y) ? maxPoint.x : maxPoint.y;
				constConvert = width / constConvert;
				maxPoint = maxPoint.multiply(constConvert);
				minPoint = minPoint.multiply(constConvert);
				constConvert *= mm2px;
				tryDraw(constConvert, maxPoint, processedLine,minPoint);
